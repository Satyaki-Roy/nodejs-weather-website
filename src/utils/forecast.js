...
const forecast = (longitude = 0, latitude = 0, callback) => {
  ...
  request({ url, json: true }, (error, { body }) => {
    if (error) {
      ...
      callback(
        undefined,
        `${body.daily.data[0].summary} It is currently ${body.currently.temperature} Degree Celsius out. There is a ${body.currently.precipProbability * 100} % chance of rain`
      );
    }
  });
};
